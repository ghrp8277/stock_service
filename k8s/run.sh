...
HELM_RELEASE_NAME="backend-stock"
HELM_NAMESPACE="prod"
...
# 로그 파일 생성 및 빌드 로그 기록 시작
{
    ...
    echo "이미지 태그: 192.168.0.212:5555/spring-stock:$CURRENT_DATE"
    echo ""

    # Docker 이미지 빌드
    echo "Docker 이미지 빌드 중..."
    docker build -t 192.168.0.212:5555/spring-stock:$CURRENT_DATE -f ../Dockerfile .. 2>&1
    BUILD_STATUS=$?
    ...
    docker push 192.168.0.212:5555/spring-stock:$CURRENT_DATE 2>&1
    PUSH_STATUS=$?
    ...
    sed -i.bak "s|image: 192.168.0.212:5555/spring-stock:.*|image: 192.168.0.212:5555/spring-stock:$CURRENT_DATE|g" $DEPLOYMENT_FILE
    UPDATE_STATUS=$?
...
if [ $STATUS -eq 0 ]; then
    ...
    rm $LOG_FILE
    docker rmi 192.168.0.212:5555/spring-stock:$CURRENT_DATE
else
